...
`remainingAmount`, `latestId` and `latestUpdatedAt` per group gives a token that
changes exactly when the book does — a fill moves `remainingAmount`, a cancel moves `count`, a
...
only `latestId`. Still enough: a group's `latestId` only ever rises. And the
fold carries `latestUpdatedAt`, which upstream describes as the one digest that
always moves when a group is touched, so a mutation that shifts none of the
counts — a status change within the same filter, a bare `updatedAt` bump — is
caught as well. That token rides along on every crawl page URL as `&v=`,
...
moves, every URL changes and the crawl runs for real. `TTL.frozen` is now a
backstop against the digest itself being wrong rather than against a mutation it
was known not to see. The cost of that precision: on self-hosted
...
- **Crafting and smelting** — 22 hand-written recipes, because nothing upstream
...
levels are separated by the experience behind them, then by their dearest
single step: levels are not a linear currency, so the same total spread over
balanced steps costs fewer bottles, and among orders tied on both the printed
one is never rougher than it has to be. Worked example: five
...
- **`makerMid` is a reference price of unknown provenance.** The upstream docs
  imply it is a mid over maker orders. It is not reproducible from the visible
  book — house best mid, quantity-weighted mid either side, house microprice and
  all-order VWAP each match at most 2 of the 33 listings carrying it — nor is it
  lifetime trade VWAP (1 of 27). Several unrelated items share exact values,
  every log reading 0.0625, which points at a configured valuation. Whether it
  is *static* is *not* established: an earlier check found it unchanged over two
